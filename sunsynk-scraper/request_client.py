...
class RequestClient():
    ...
    def login(self):
        ...
        logging.info("get_bearer_token")
        logging.info(credentials.bearer_token)

    def setup_plant(self):
        credentials.my_plant_id = self.get_plant_id()
        logging.info("get_plant_id")
    ...
    def get_bearer_token(self):
        ...
        if credentials.sunsynk_region == 1:
            raw_data = requests.post(endpoints.login_endpoint_r1, json=payload, headers=headers).json()
        else:
            raw_data = requests.post(endpoints.login_endpoint_r2, json=payload, headers=headers).json()
    ...
    def get_plant_id(self):
        logging.info("getting plant id")
        logging.info(self.get_headers_and_token())
        if credentials.sunsynk_region == 1:
            r = requests.get(endpoints.plants_endpoint_r1, headers=self.get_headers_and_token())
        else:
            r = requests.get(endpoints.plants_endpoint_r2, headers=self.get_headers_and_token())
    ...
    def get_monthly_readings(self):
        if credentials.sunsynk_region == 1:
            ...
            path = endpoints.get_month_readings_endpoint_r2(credentials.my_plant_id, datetime.date.today())
            response = self.get(path)
            return response.json()
